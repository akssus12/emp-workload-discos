...
int main(int argc, char** argv) {
    ...
    int j;
    #pragma omp parallel shared(array_word, hash_table) private(i, start_string, end_string, string, token, j, is_alpha, free_string) num_threads(2)
    {
        i = omp_get_thread_num();
        is_alpha = true;
    ...
    #pragma omp parallel shared(hash_table, words) private(i)
    {
        ...
        if(id == total_thread -1)
            h_end = hash_table->size;

        int c_count = 0; 
        for(i=h_start; i<h_end; i++){
            c_count += hash_table->node_count[i]; 
        }
        ...
        if(id == 0){
            ...
            c_start = hash_table->count - c_count;
