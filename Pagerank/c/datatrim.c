...
/*
The script to fetch the subset of the original data and add links to the node with no 
out going links The index is remapped to make it continuous in the 
resulting subset of data. Index start from 0.

As for the output file, first line is the number of the included nodes, the following 
lines indicating the directed links with the first number as the index 
...
Options:
    ...
    -n    tag to shut down the auto link addition for the nodes that have no out going 
    links

-----
Outputs:
    Output files:
    data_input_link:    the directed links with the first number as the index of the 
    source node and the second number as the index of the destination node.
    data_input_meta:    first line indicating the number of the nodes, the following 
    lines indicating the node index, number of incoming links, number of 
    outgoing links. 
